...
class project{
    ...
    function breadcrumb(){
        ...
        if(isset($_GET['id'])){
            ...
            $item=$this->db->getOne('project','id,title,e_title');
            $str.='
    ...
    function category($pId){
        ...
        $list=$this->db->get('project_cate',null,'id,title,e_title');
        $str='
        <ul class="category">';
        foreach($list as $item){
            $title=$this->lang=='vi'?$item['title']:$item['e_title'];
            if($item['id']==$pId) $cls=' class="active"';else $cls='';
            $lnk=myWeb.$this->view.'/'.common::slug($title).'-p'.$item['id'];
            $str.='
            <li><a'.$cls.' href="'.$lnk.'"><i class="fa fa-angle-double-right"></i> '.$title.'</a></li>';
        }
        $str.='
        </ul>';
        return $str;
